...
def make_img(user_id, user_name, mode="lost"):
    ...
    if show_weights:
        ...
        try:
            ...
            if diff_pp_f > 0: diff_color = GREEN_COLOR
            elif diff_pp_f < 0: diff_color = RED_COLOR
            else: diff_color = COLOR_WHITE
        except ValueError:
            new_diff_pp = "N/A"
        ...
        try:
            ...
            if acc_delta_f > 0: acc_diff_color = GREEN_COLOR
            elif acc_delta_f < 0: acc_diff_color = RED_COLOR
            else: acc_diff_color = COLOR_WHITE
        except ValueError:
            pot_acc_str = "N/A"
            acc_diff_str = "N/A"
            acc_diff_color = COLOR_WHITE
    ...
    if mode == "lost": baseline_offset = 20
    ...
    for i, row in enumerate(rows):
        ...
        if not all([raw_artist, raw_title, creator, version]) and beatmap_id.isdigit():
             try:
                                                                          
                 bdata = get_map_osu(beatmap_id, token)
                 if bdata:
                      raw_artist = bdata.get("artist", raw_artist)
                      raw_title = bdata.get("title", raw_title)
                      creator = bdata.get("creator", creator)
                      version = bdata.get("version", version)
                      if bdata.get("beatmapset"):
                          cover2x = bdata["beatmapset"]["covers"].get("cover@2x")
             except Exception as map_err:
                  logger.warning(f"Ошибка получения данных карты {beatmap_id}: {map_err}")
        ...
        if cover2x:
            ...
            try:
                ...
                c_img = Image.open(cover_file).convert("RGBA").resize((cover_w, cover_h_))
                 ...
                 logger.warning(f"Не удалось загрузить/открыть обложку {cover_file}: {cover_err}")
                 c_img = None                          
        ...
        d_card.rounded_rectangle((shape_left, card_y, shape_left + shape_w, card_y + card_h), radius=15, fill=PP_SHAPE_COLOR)
                        
        raw_pp = row.get("PP", "0")
        try: pp_val = round(float(raw_pp))
        except ValueError: pp_val = 0
        pp_str = f"{pp_val}pp"
        try:                                   
        ...
        if not show_weights:                    
            ...
            acc_x = right_block_x - 80                        
            raw_acc_str = row.get("Accuracy", "0")
            try: acc_s = f"{float(raw_acc_str):.2f}%"
            except ValueError: acc_s = f"{raw_acc_str}%" if raw_acc_str else "?.??%"
            d_card.text((acc_x, center_line - 10), acc_s, font=BOLD_ITALIC_FONT, fill=ACC_COLOR, anchor="lm")                              

                           
            mods_right_edge = acc_x - 10                     
            ...
            wpp_x = right_block_x - 70                           
            raw_wpp = row.get("weight_PP", "")
            try: weight_pp_text = f"{round(float(raw_wpp))}pp"
            except ValueError: weight_pp_text = ""
            if weight_pp_text:
                d_card.text((wpp_x, center_line - 8), weight_pp_text, font=BOLD_ITALIC_FONT_SMALL, fill=WEIGHT_COLOR, anchor="lm")

                                   
            acc_block_right_edge = wpp_x - 20                        
            acc_block_width = 80
            acc_block_x = acc_block_right_edge - acc_block_width
                      
            raw_acc = row.get("Accuracy", "0")
            try: acc_str2 = f"{float(raw_acc):.2f}%"
            except ValueError: acc_str2 = f"{raw_acc}%" if raw_acc else "?.??%"
            d_card.text((acc_block_x, center_line - 14), acc_str2, font=BOLD_ITALIC_FONT, fill=ACC_COLOR, anchor="lm")
                        
            raw_wpercent = row.get("weight_%", "")
            try: w_percent_str = f"weighted {round(float(raw_wpercent))}%"
            except ValueError: w_percent_str = ""
            if w_percent_str:
                d_card.text((acc_block_x, center_line + 8), w_percent_str, font=CREATOR_SMALL_FONT, fill=WEIGHT_COLOR, anchor="lm")

                           
            mods_right_edge = acc_block_x - 10                           
            mods_list = short_mods(row.get("Mods", ""))
            mod_x_cur = mods_right_edge
...
def make_img_lost(user_id=None, user_name=""):                          
    make_img(user_id=user_id, user_name=user_name, mode="lost")


def make_img_top(user_id=None, user_name=""):                          
