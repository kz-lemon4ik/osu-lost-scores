...
class BrowserOAuthFlow:
        ...
        class CallbackHandler(BaseHTTPRequestHandler):
            def do_GET(self):
                try:
                    ...
                    if 'jwt_token' in query_params and 'user_id' in query_params and 'username' in query_params:
                        jwt_token = query_params['jwt_token'][0]
                        user_id = int(query_params['user_id'][0])
                        username = query_params['username'][0]
                        
                        flow.auth_manager.save_oauth_session(jwt_token, user_id, username)
    ...
    def start_login(self):
        try:
            ...
            self.callback_server = HTTPServer(('localhost', self.callback_port), handler_class)
            
            server_thread = threading.Thread(target=self.callback_server.serve_forever, daemon=True)
            server_thread.start()
        ...
        try:
            while time.time() - start_time < timeout_seconds:
                if self.received_token:
                    ...
                    username = session.username if session and session.username else "Unknown"
